...
public class Bank
{
    ...
    //getOverdraft, gets the current overdraft of the bank account if it is an OverdraftAccount
    public int getOverdraft() {
        //Check if the account is an overdraft account
        if (loggedIn() && account instanceof OverdraftAccount) {
            //Return the overdraft
            return ((OverdraftAccount) account).getOverdraft();   
        } else {
            return -1; // use -1 as an indicator of an error
        }
    }
    
    //hasOverdraft, returns if the current account is a OverdraftAccount and subsiquently if it has an overdraft
